...
# Family 3
ids=($F3 $F1 $F2 $F4) # change the order to match sample order and not the age of individual order
## construct the locations of the input files
file_patterns=( "${ids[@]/%/.combined.genotyped.vcf}")
file_patterns=( "${file_patterns[@]/#/${SUPP3DIR}/SS400}")
...
vcf-sort -c \
        ...
        > sample_merged_sorted_annotated_family3.vcf

cd - # change back to the directory we started from (family directory)

# Now extract the relevant information from the vcf files and post-process
module load python
python scripts/survivor_extract.py \
        "${SURVIVOR_WORKDIR}/sample_merged_sorted_annotated_family2.vcf" \
        101 > data/output/family2/cnv_parliament2.tsv
python scripts/survivor_extract.py \
        "${SURVIVOR_WORKDIR}/sample_merged_sorted_annotated_family3.vcf" \
