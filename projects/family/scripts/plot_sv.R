...
truth.vcf.cmd <- paste("grep -v \"#\" data/input/NA12878_test.combined.genotyped.vcf")

x <- read.table(pipe(truth.vcf.cmd), stringsAsFactors=FALSE)
colnames(x) <- c("CHROM", "POS", "ID", "REF", 
                 "ALT", "QUAL", "FILTER", "INFO", 
                 "FORMAT", "SAMPLEINFO")

subx <- x %>% 
  extract(INFO, c("SUPP", "END"), 
                "SUPP=([[:digit:]]+);.*;END=([[:digit:]]+);") %>%
  filter(SUPP >= 2) %>% 
  identity()
...
centromeres[["chromosome"]] <- factor(x = centromeres[["chromosome"]],
                                      levels = chrom_order)
subx[["chromosome"]] <- factor(x=subx[["CHROM"]], levels=chrom_order)
...
ggplot(data=chrom_sizes) + 
    ...
    geom_rect(data= subx %>% filter(SUPP >= 4), aes(
                 xmin=as.numeric(chromosome) - 0.2,
                 xmax=as.numeric(chromosome) + 0.2,
                 ymin=POS,
                 ymax=as.integer(END), 
                 color=as.factor(ALT), fill=as.factor(ALT), 
                        alpha=0.5) ) +
    theme( 
    ...
    ggtitle(paste0("Parliament 2 results when four callers agree" ))


ggsave("~/parliament2.png")    

# Let's  look at one chromosome
# the deletions ground truth
truth.del <- read.table("data/input/NA12878_test.truth.txt", 
                    header=TRUE, sep="\t") %>%
         transmute(chromosome=paste0("chr", Chromosome), 
                   POS=Start.position, END=End.position) 

parliament.del <- subx %>% 
        filter(ALT == "<DEL>", FILTER=="PASS") %>%
        select(chromosome, POS, END, QUAL, SUPP)


ggplot(data=parliament.del %>% 
            filter(chromosome=="chr1") %>%
            filter(SUPP >=3)
       )  +
    geom_rect(aes(xmin = as.numeric(POS) - 0.2,
                  xmax = as.numeric(END) + 0.2,
                  ymax = 0.35, ymin = 0.55, fill="PARLIAMENT"), 
              color="blue") +
    geom_rect(data= truth.del %>% filter(chromosome == "chr1"), 
              aes(
                 xmin=as.numeric(POS) - 0.2,
                 xmax=as.numeric(END) + 0.2,
                 ymin=0.1,
                 ymax=0.3, fill="TRUTH"), color="red") +
    scale_fill_manual(name="", 
                      values=c("PARLIAMENT"="blue", 
                               "TRUTH"="red")) +
    scale_x_continuous( 
        name="Chr Position",
        labels=scales::unit_format(unit="Mb", scale=1e-6)) + 
    scale_y_discrete() +
    labs(title="Chr1 deletions") 


ggsave("~/parliament.chr1.png", height=2, width=10)
