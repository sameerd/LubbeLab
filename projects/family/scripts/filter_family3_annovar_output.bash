...
cut -f1-7,9,10,11,20 ${OUTPUT_TXT} \
      | paste - <(cut -f33 "${OUTPUT_TXT}" | awk -F: 'NR == 1 {print "F1"}; NR > 1 {print $1}') \
      | paste - <(cut -f34 "${OUTPUT_TXT}" | awk -F: 'NR == 1 {print "F2"}; NR > 1 {print $1}') \
      | paste - <(cut -f35 "${OUTPUT_TXT}" | awk -F: 'NR == 1 {print "F3"}; NR > 1 {print $1}') \
      | paste - <(cut -f36 "${OUTPUT_TXT}" | awk -F: 'NR == 1 {print "F4"}; NR > 1 {print $1}') \
      > ${OUTPUT_TXT_SMALL}
